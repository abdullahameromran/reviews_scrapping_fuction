    ...
    try:
        ...
        if not scraping_metadata_id:
            return {
                'statusCode': 400,
                'headers': CORS_HEADERS,
                'body': json.dumps({'error': 'scraping_metadata_id is required in the request body'})
            }
        ...
        supabase_url = "https://tqgqhkeiqxtfvlquojqz.supabase.co"
        supabase_key = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InRxZ3Foa2VpcXh0ZnZscXVvanF6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDk0MTM0OTIsImV4cCI6MjA2NDk4OTQ5Mn0.4KzvEd7AKY8y76pJhAw5j8fcg90qZZmPBCJNkWOJMcI"
...
def handle_synchronous_scraping(scraping_attempt_id, place_ids, max_reviews, reviews_start_date, business_place_id, supabase_url, supabase_headers, user_profile_id):
    """Handle the actual scraping work"""
    try:
        # Check Apify token
        apify_token = "apify_api_YB1nTWm3VdvdZcvC7qMr9geaSLHNbo2NtjzQ"
        if not apify_token:
            try:
                requests.patch(
                    f'{supabase_url}/rest/v1/scraping_metadata?id=eq.{scraping_attempt_id}',
                    ...
                    data=json.dumps({
                        'scraping_status': 'failed',
                        'error_message': 'Apify token not configured'
                    }),
                    timeout=30
                ).raise_for_status()
            except:
            ...
            return {
                'statusCode': 500,
                'headers': CORS_HEADERS,
                'body': json.dumps({'error': 'Apify token not configured'})
            }
                        ...
                        if review_id:
                            try:
                                requests.patch(
                                    ...
                                    headers=supabase_headers,
                                    data=json.dumps({'scrapping_attempt': scraping_attempt_id}),
                                    timeout=30
                                ).raise_for_status()
        ...
        response = requests.post(
            f'{supabase_url}/rest/v1/business',
            headers=supabase_headers,
